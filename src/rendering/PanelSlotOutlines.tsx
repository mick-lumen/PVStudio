...
function PanelSlot({ slot, onAdd }: { readonly slot: PanelSlotOutline; readonly onAdd?: (placement: PanelPlacement) => void }): ReactNode {
  const pose = useMemo(
  ...
  const handlePointerDown = (event: ThreeEvent<PointerEvent>): void => {
    if (event.button !== 0 || onAdd === undefined) return
    event.stopPropagation()
    onAdd(slot.placement)
  }
  return (
    <group data-testid="panel-slot" matrix={matrix} matrixAutoUpdate={false} onPointerDown={handlePointerDown}>
      <mesh position={[0, height / 2, lift]} renderOrder={20}><boxGeometry args={[width + stroke, stroke, stroke]} /><meshBasicMaterial color="#ffffff" transparent opacity={0.92} depthWrite={false} /></mesh>
      <mesh position={[0, -height / 2, lift]} renderOrder={20}><boxGeometry args={[width + stroke, stroke, stroke]} /><meshBasicMaterial color="#ffffff" transparent opacity={0.92} depthWrite={false} /></mesh>
      <mesh position={[width / 2, 0, lift]} renderOrder={20}><boxGeometry args={[stroke, height + stroke, stroke]} /><meshBasicMaterial color="#ffffff" transparent opacity={0.92} depthWrite={false} /></mesh>
      <mesh position={[-width / 2, 0, lift]} renderOrder={20}><boxGeometry args={[stroke, height + stroke, stroke]} /><meshBasicMaterial color="#ffffff" transparent opacity={0.92} depthWrite={false} /></mesh>
...
export function PanelSlotOutlines({ slots, onAdd }: PanelSlotOutlinesProps): ReactNode {
  return (
    <group name="pv-panel-slot-outlines">
      {slots.map((slot) => <PanelSlot key={slot.placement.id} slot={slot} onAdd={onAdd} />)}
    </group>
  )
}
